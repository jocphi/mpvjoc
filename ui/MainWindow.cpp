...
void MainWindow::playNextPlaylistFile(){if(playlistModel->count()<=0)return; int r=currentRow(); if(r<0)r=0; r=(r+1)%playlistModel->count(); playPlaylistRow(r);} void MainWindow::playPreviousPlaylistFile(){if(playlistModel->count()<=0)return; int r=currentRow(); if(r<0)r=0; r=(r-1+playlistModel->count())%playlistModel->count(); playPlaylistRow(r);} QString MainWindow::moveConfigFilePath()const{QFileInfo info(stateFilePath()); return QDir(info.absolutePath()).filePath("move-buttons.conf");} QString MainWindow::moveLogFilePath()const{QFileInfo info(stateFilePath()); return QDir(info.absolutePath()).filePath("move-log.tsv");}
...
void MainWindow::fastPlaybackTick(){if(!warpPlaybackMode||!mpvWidget)return;if(mpvWidget->currentFilePath().isEmpty()){warpPlaybackMode=false;if(fastPlaybackTimer)fastPlaybackTimer->stop();updateWarpOverlay();return;}mpvWidget->seekRelativeKeyframe(10*warpFactor);}
...
void MainWindow::updateTimeLabel(double p,double d){if(timeLabel)timeLabel->setText(formatHMS(p)+" / "+formatHMS(d));} void MainWindow::openFiles(){addFiles(QFileDialog::getOpenFileNames(this,"Open media files",{},"Media files (*.mkv *.mp4 *.avi *.mov *.webm *.mp3 *.flac *.wav);;All files (*)"));}
void MainWindow::onPositionChanged(double s){position=s;timeline->setPosition(s);updateTimeLabel(position,duration);} void MainWindow::onDurationChanged(double s){duration=s;timeline->setDuration(s);playlistModel->setDurationForPath(mpvWidget->currentFilePath(),s);savePlaylistState();updateTimeLabel(position,duration);} void MainWindow::removeSelectedItem(){int r=currentRow(); if(playlistModel->removeRowAt(r)&&playlistModel->count()>0)playlistView->setCurrentIndex(playlistModel->index(qBound(0,r,playlistModel->count()-1),0)); savePlaylistState();} void MainWindow::clearPlaylist(){playlistModel->clearItems();savePlaylistState();} void MainWindow::moveSelectedItemUp(){int r=currentRow(); if(playlistModel->moveRowUp(r))playlistView->setCurrentIndex(playlistModel->index(r-1,0));savePlaylistState();} void MainWindow::moveSelectedItemDown(){int r=currentRow(); if(playlistModel->moveRowDown(r))playlistView->setCurrentIndex(playlistModel->index(r+1,0));savePlaylistState();}
