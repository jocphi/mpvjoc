...
MpvWidget::MpvWidget(QWidget*p):QOpenGLWidget(p){forceCLocaleForMpv();setMinimumSize(640,360);setFocusPolicy(Qt::StrongFocus); mpv=mpv_create(); if(!mpv)qFatal("no mpv"); check_mpv_error(mpv_set_option_string(mpv,"terminal","yes")); check_mpv_error(mpv_set_option_string(mpv,"msg-level","all=warn,libmpv_render=fatal")); QString savedHwdecMode=QSettings().value(QStringLiteral("video/hwdecMode")).toString().trimmed(); if(savedHwdecMode.isEmpty()){const bool legacyHardwareDecoding=QSettings().value(QStringLiteral("video/hardwareDecoding"),false).toBool();savedHwdecMode=legacyHardwareDecoding?QStringLiteral("auto-safe"):QStringLiteral("no");} const QByteArray savedHwdecUtf8=savedHwdecMode.toUtf8(); check_mpv_error(mpv_set_option_string(mpv,"hwdec",savedHwdecUtf8.constData())); check_mpv_error(mpv_set_option_string(mpv,"video-unscaled","yes")); check_mpv_error(mpv_set_option_string(mpv,"vo","libmpv")); check_mpv_error(mpv_set_option_string(mpv,"input-default-bindings","no")); check_mpv_error(mpv_set_option_string(mpv,"osc","no")); check_mpv_error(mpv_initialize(mpv)); mpv_observe_property(mpv,1,"time-pos",MPV_FORMAT_DOUBLE); mpv_observe_property(mpv,2,"duration",MPV_FORMAT_DOUBLE); mpv_observe_property(mpv,3,"pause",MPV_FORMAT_FLAG); mpv_observe_property(mpv,4,"path",MPV_FORMAT_STRING); mpv_observe_property(mpv,5,"volume",MPV_FORMAT_DOUBLE); mpv_observe_property(mpv,6,"mute",MPV_FORMAT_FLAG); mpv_observe_property(mpv,7,"dwidth",MPV_FORMAT_INT64); mpv_observe_property(mpv,8,"dheight",MPV_FORMAT_INT64); mpv_set_wakeup_callback(mpv,wakeup,this); updateIdleLogoOverlay();}
...
void MpvWidget::setHardwareDecodingMode(const QString&mode){
    if(!mpv)return;
    const QString normalized=mode.trimmed().isEmpty()?QStringLiteral("no"):mode.trimmed();
    const QByteArray value=normalized.toUtf8();
    mpv_set_property_string(mpv,"hwdec",value.constData());
}
