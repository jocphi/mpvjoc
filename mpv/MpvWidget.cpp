...
void MpvWidget::setMaxVideoScale(double scale){ double clamped=qBound(0.5,scale,2.0); double zoom=std::log2(clamped); QByteArray z=QByteArray::number(zoom,'f',3); const char*cmd[]={"set","video-zoom",z.constData(),nullptr}; check_mpv_error(mpv_command_async(mpv,0,cmd));}
